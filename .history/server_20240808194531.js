...
const https = require('https'); // Add https module for making HTTP requests
const http = require('http'); // Add http module for making HTTP requests

// Initialize Firebase Admin SDK
admin.initializeApp({
  credential: admin.credential.applicationDefault(),
  storageBucket: 'buckeTest'
});
...
const uri = 'mongodb+srv://dev:64659$@cluster0.snn3y.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0';
...
async function uploadToFirebase(fileUri, folder) {
  ...
  // Use http/https modules to fetch the file
  const isHttps = fileUri.startsWith('https://');
  const protocol = isHttps ? https : http;

  return new Promise((resolve, reject) => {
    protocol.get(fileUri, (response) => {
      if (response.statusCode !== 200) {
        reject(new Error(`Failed to get '${fileUri}' (${response.statusCode})`));
        return;
      }
      ...
      response.on('end', async () => {
        const stream = Readable.from(Buffer.concat(buffer));
        try {
