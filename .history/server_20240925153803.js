...
app.delete('/delete_attendee/:event_id', asyncHandler(async (req, res) => {
  try {
    await Event.deleteOne({ eventId: req.params.event_id });
    res.json({ message: 'Event deleted' }); 
  } catch (error) {
...
app.put('/edit_attendee/:event_id/:userI_id', asyncHandler(async (req, res) => {

  try {
    const updates = req.body;
    const updatedSession = await User.findOneAndUpdate({ sessionId: req.params.session_id }, { $set: updates }, { returnDocument: 'after' });
