...
app.post('/auth/signup',  upload.single('userImage'), asyncHandler(async (req, res) => {
  ...
  const { username, email, password, job, gender, industry, phone } = req.body;
  ...
  try {
    ...
    const user = { id: uuidv4(), username: username, email: email, password: hashedPassword, userImage: imagePath, phone:phone, job:job, gender:gender, industry:industry };
