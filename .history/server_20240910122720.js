...
// User Management
app.get('/all_users/profile', authenticate, asyncHandler(async (req, res) => {
  const user = await User.findOne({ id: req.auth.userId });
  res.json(user);
}));

app.put('/all_users/profile', authenticate, asyncHandler(async (req, res) => {
  const updates = req.body;
  const updatedUser = await User.findOneAndUpdate({ id: req.auth.userId }, { $set: updates }, { returnDocument: 'after' });
  res.json(updatedUser.value);
}));

app.get('/all_users/:user_id', asyncHandler(async (req, res) => {
