...
app.get('/messages', async (req, res) => {
  try {
    // Retrieve all messages from the 'chat' collection
    ...
    // Convert Mongoose documents to plain JavaScript objects and remove circular references
    const sanitizedMessages = messages.map((msg) => msg.toObject({ getters: true, versionKey: false }));
...
// Assuming you have a MongoDB client instance connected as `client`
app.post("/save_checkin2", asyncHandler(async (req, res) => {
  ...
  // Ensure checkins array exists and is treated as an array
  ...
  try {
    ...
    console.log(`user checked at${new }`)
...
app.post("/check_duplicate", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const existingCheckin = await Checkins.findOne({ attendeeEmail:userEmail });
...
app.post('/messageCount', async (req, res) => {
  ...
  try {
      ...
      if (result.modifiedCount > 0) {
          res.status(200).send({ message: 'Message count updated successfully' });
      } else {
...
app.post('/contactShareCount', async (req, res) => {
  console.log("shared message Count with body:", req.body); // Debugging line

  const { eventId, increment } = req.body; // assuming we want to increment the count
